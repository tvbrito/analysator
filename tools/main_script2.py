...
# options for what will be done during the calculations, 1 IS YES, 0 (or anything else than 1) is no
...
if read_from_vlsv == 1:
   ...
   variables={'B':0, 'E':0, 'rho':0, 'v':0,'Pressure':0}
   #retunrs vector fields as components Bx, By, Bz and additionally time, cellids and coordinates
   variables=rd.read_from_vlsv_dictionary(path, path_to_vlsv, variables, use_coordinates, points_in_RE, interpolate, points_for_interpolation, start, end, step)
...
# --- IF YOU WANT TO READ FROM  TXT FILES THAT WERE SAVED EARLIER ---
   ...
   variables={'Bx':0, 'By':0, 'Bz':0, 'Ex':0, 'Ey':0,'Ez':0, 'rho':0, 'vx':0, 'vy':0,'vz':0, 'Temperature':0, 'TPerpendicular':0, 'TParallel':0, 'betaPerpendicular':0, 'betaParallel':0, 'Pressure':0,'coordinate_x':0,'coordinate_y':0,'coordinate_z':0, 'time':0 }

   variables=rd.read_from_local_file_dictionary(path, variables)
...
#p.plot_as_a_function_of_distance_at_each_time(path, variable, distance, time, time_factor, time_step_for_saving)
#p.plot_as_a_function_of_time_at_each_point(path, variable, distance, time, time_factor)
